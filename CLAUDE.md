...
- **Module gating**: `shule/modules.py` `enabled_modules()` / `module_enabled()` are the only source of which optional modules are on. It is the admin's choice (Admin Panel → Modules, stored in `SchoolSettings.enabled_modules`, cached) narrowed to `LICENSED_MODULES` (env, set per school at setup; unset = all). Until an admin saves, the `ENABLED_MODULES` env value stands in for the admin's choice. Never read `settings.ENABLED_MODULES` directly. `OPTIONAL_MODULES` is the canonical list, and `MODULE_INFO` / `MODULE_REQUIRES` feed the admin page. Gating happens in three places, and all of them need updating when you add or gate a feature:
  1. Backend: set `module = '<key>'` on the view and add `ModuleEnabled` to `permission_classes`. Celery tasks and signals call `module_enabled()` themselves.
  ...
  Tests use `@override_settings(ENABLED_MODULES=[...])` (and `LICENSED_MODULES`). A test that saves `SchoolSettings.enabled_modules` must clear the module cache around itself (`accounts.tests_modules.ModuleCacheMixin`), because the cache outlives the test's rolled-back transaction.
