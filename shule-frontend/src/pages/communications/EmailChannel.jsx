...
const CHANNELS = [
  { value: 'EMAIL', label: 'Email', Icon: Mail,       hint: null },
  { value: 'SMS',   label: 'SMS',   Icon: Smartphone, hint: 'Open the SMS tab' },
]
        ...
        <div className="grid grid-cols-2 gap-3">
          {CHANNELS.map(({ value, label, Icon, hint }) => {
            ...
            return (
              <button
                key={value}
                onClick={() => (isEmail ? setChannel('EMAIL') : onSwitchToSms?.())}
                className={`flex flex-col items-center gap-1 py-3.5 px-2 rounded-xl
...
export default function EmailChannel({ onSwitchToSms }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'compose' && <ComposeTab onSwitchToSms={onSwitchToSms} />}
