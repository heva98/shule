  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {canCreate && (
          <Button onClick={() => setShowAdd(true)} icon={Plus} className="shrink-0">
            Post Package
          </Button>
        )}
      </div>
      ...
      {isLoading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => (
            <Card key={i} padding="p-4" className="animate-pulse h-32" />
          ))}
        </div>
      ) : packages.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {packages.map((pkg) => {
            ...
            return (
              <Card key={pkg.id} padding="p-4" className="flex flex-col">
